...
status_e big_to_little32(uint32_t * data, uint32_t length)
{
	...
	if(data==NULL)
        ...
        while(length--)		// loop to change endian-ness of all the elements
        {
                ...
                i = 3;		// lopp variable value to 7 to change nibble
                while(i--)
                {
                        temp |= (*data & 0xFF);		// taking out last nibble of the data
                        *data = *data >>8;		// left shift data by 4 to take out the next nibble
                        temp = i ? temp<<8: (temp<<8 | *data);	// in the last loop, temp is shifted and ORed with *data
                }
